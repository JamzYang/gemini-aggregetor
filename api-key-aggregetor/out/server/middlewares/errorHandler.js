...
const logger_1 = require("./logger"); // 导入日志记录器
const GoogleApiForwarder_1 = require("../core/GoogleApiForwarder"); // 导入自定义错误类型
const errorHandler = (err, req, res, next) => {
    // 记录详细错误日志
    logger_1.logger.error({ err, req }, `处理请求时发生错误: ${err.message}`);
    let statusCode = 500;
    let message = 'Internal Server Error';
    let status = 'INTERNAL'; // 映射到 Google API 的错误状态
    if (err instanceof GoogleApiForwarder_1.GoogleApiError) {
