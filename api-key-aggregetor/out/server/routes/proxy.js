...
exports.default = createProxyRouter;
const express_1 = require("express");
const config_1 = __importDefault(require("../config"));
// 修改为导出一个函数，接受依赖作为参数
function createProxyRouter(apiKeyManager, requestDispatcher, googleApiForwarder, streamHandler) {
    const router = (0, express_1.Router)();
    // 定义代理路由，匹配 Gemini API 的 generateContent 路径
    // 定义代理路由，匹配 Gemini API 的 models/{model}:{method} 路径
    // 使用正则表达式捕获 model 和 method
    router.post(/^\/v1beta\/models\/([^:]+):([^:]+)$/, async (req, res, next) => {
        let apiKey = null;
        try {
            ...
            if (forwardResult.error) {
                ...
                if (err.isRateLimitError) {
                    // 如果是速率限制错误，标记 Key 冷却
                    apiKeyManager.markAsCoolingDown(apiKey.key, config_1.default.KEY_COOL_DOWN_DURATION_MS);
                    // TODO: 实现可选的重试逻辑
                    // 目前将错误传递给错误处理中间件
                    next(err);
                }
